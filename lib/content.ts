...
function loadData(): Record<ContentType, ContentItem[]> {
  ...
  try {
    ...
    interface CalloutToken {
      type: "callout"; raw: string; calloutType: string; foldMarker: string; title: string; tokens: unknown[];
    }
    const calloutExtension = {
      ...
      tokenizer(this: { lexer: { blockTokens: (src: string, tokens: unknown[]) => unknown } }, src: string): CalloutToken | undefined {
        ...
        const token: CalloutToken = { type: "callout", raw: match[0], calloutType: type, foldMarker, title, tokens: [] };
