/**
 * Single source of truth for markdown → HTML rendering.
 * Used by both:
 *   - scripts/build-content.mjs (build-time, Node ESM)
 *   - lib/content.ts            (dev-time fs fallback, compiled by Next)
 *
 * To change how markdown renders (callout types, link rewriting, heading
 * anchors, new extensions), edit THIS file only.
 */

/** Heading text → anchor-safe id. Keeps ASCII word chars, CJK, hyphens. */
...
const FOLD_CHEVRON = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>';
...
const calloutExtension = {
  ...
  tokenizer(src) {
    ...
    const token = { type: "callout", raw: match[0], calloutType: type, foldMarker, title, tokens: [] };
  ...
  renderer(token) {
    ...
    const titleInner = `<span class="callout-icon" aria-hidden="true">${icon}</span>`
      ...
      + (foldable ? `<span class="callout-fold" aria-hidden="true">${FOLD_CHEVRON}</span>` : "");
    if (foldable) {
...
// ── Download card (```download fenced block) ────────────────────────────────
const PDF_ICON_SVG = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><text x="12" y="17" text-anchor="middle" font-size="5.5" font-weight="700" stroke="none" fill="currentColor" font-family="ui-sans-serif,system-ui,sans-serif">PDF</text></svg>';

const DOWNLOAD_ICON_SVG = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>';

const EYE_ICON_SVG = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z"/><circle cx="12" cy="12" r="3"/></svg>';

const downloadCardExtension = {
  name: "downloadCard",
  level: "block",
  start(src) {
    const m = src.match(/^```download\b/m);
    return m ? m.index : undefined;
  },
  tokenizer(src) {
    const rule = /^```download[ \t]*\n([\s\S]*?)\n```[ \t]*(?:\n|$)/;
    ...
    return { type: "downloadCard", raw: match[0], fields };
  },
  renderer(token) {
    const f = token.fields;
    const title = escapeAttr(f.title || "");
    const subtitle = escapeAttr(f.subtitle || "");
    const desc = escapeAttr(f.desc || "");
    const meta = escapeAttr(f.meta || "");
    const file = f.file || "";
    const fileHref = escapeAttr(encodeURI(file));
    const readHref = escapeAttr(encodeURI(f.read || file));
    const downloadName = file.split("/").pop() || "";
    const downloadAttr = downloadName ? ` download="${escapeAttr(downloadName)}"` : " download";
    return ''
      + '<div class="download-card">'
      + `<div class="download-card__icon" aria-hidden="true">${PDF_ICON_SVG}</div>`
      + '<div class="download-card__body">'
      +   (title    ? `<div class="download-card__title">${title}</div>` : "")
      +   (subtitle ? `<div class="download-card__subtitle">${subtitle}</div>` : "")
      +   (desc     ? `<div class="download-card__desc">${desc}</div>` : "")
      +   (meta     ? `<div class="download-card__meta">${meta}</div>` : "")
      +   '<div class="download-card__actions">'
      +     `<a class="download-card__btn download-card__btn--secondary" href="${readHref}" target="_blank" rel="noopener noreferrer">`
      +       `<span class="download-card__btn-icon" aria-hidden="true">${EYE_ICON_SVG}</span>`
      +       '<span>在线阅读</span>'
      +     '</a>'
      +     `<a class="download-card__btn download-card__btn--primary" href="${fileHref}"${downloadAttr}>`
      +       `<span class="download-card__btn-icon" aria-hidden="true">${DOWNLOAD_ICON_SVG}</span>`
      +       '<span>下载 PDF</span>'
      +     '</a>'
      +   '</div>'
      + '</div>'
...
/**
 * Wrap markdown link URLs that contain spaces in angle brackets so the parser
 * treats them as valid links. [text](url with spaces) → [text](<url with spaces>)
 */
export function preprocessMarkdown(src) {
  return src.replace(/\[([^\]]*)\]\(([^)<>]*[ ][^)<>]*)\)/g, (match, text, url) => {
    return `[${text}](<${url}>)`;
  });
}

/**
 * Apply all custom renderers and extensions to a marked instance.
 * Pass the marked module and its Renderer class.
 */
export function applyMarkdownConfig(marked, Renderer) {
  ...
  renderer.link = function ({ href, title, text, tokens }) {
    ...
    return `<a href="${escapeAttr(encodeURI(href))}"${titleAttr}>${html}</a>`;
  };
  ...
  marked.use({ renderer });
  marked.use({ extensions: [calloutExtension, downloadCardExtension] });
}
